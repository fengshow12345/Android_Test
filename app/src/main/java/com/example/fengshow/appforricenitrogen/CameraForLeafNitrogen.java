...
import com.example.fengshow.appforricenitrogen.image.CustView;
...
public class CameraForLeafNitrogen extends Activity implements SurfaceHolder.Callback {
...
//    private TextView tvVert;
    ...
    private CustView mDrawCV;

    private Camera mCamera1;
    private SurfaceView mPreview1;
    private SurfaceHolder mHolder1;
    private Camera.PictureCallback mpictureCallBack1 = new Camera.PictureCallback() {
        @Override
        public void onPictureTaken(byte[] data, Camera camera) {
            //byte[] data保存着拍照的完整数据
            File tempFile = new File("/sdcard/temp-for-Leaf.jpg");
            ...
            try {
                ...
                //传递
                Intent intent = new Intent(CameraForLeafNitrogen.this, ResultActivityForLeaf1.class);
                intent.putExtra("picPath_leaf", tempFile.getAbsolutePath());
                startActivity(intent);
                CameraForLeafNitrogen.this.finish();//将拍照的Activity finish掉
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.camera_for_leaf_nitrogen);
...
//        sensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);
        ...
        //
        mPreview1 = (SurfaceView) findViewById(R.id.sfv_Camera_for_Leaf);
        mHolder1 = mPreview1.getHolder();
        mHolder1.addCallback(this);//获取SurfaceView
        mPreview1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mCamera1.autoFocus(null);
            }
        });
        //绘制矩形
        mDrawCV= (CustView) findViewById(R.id.drawCV);
        mDrawCV.draw(new Canvas());
    }

    //设置相机参数
    public void captureleaf(View view) {
        ...
        mCamera1.autoFocus(new Camera.AutoFocusCallback() {
            ...
            public void onAutoFocus(boolean success, Camera camera) {
                //判断对焦是否完全准确，然后拍照
                if (success) {
                    mCamera1.takePicture(null, null, mpictureCallBack1);
                }
            }
        });
    }
...
//        sensorManager.registerListener(this, mag_sensor, SensorManager.SENSOR_DELAY_NORMAL);
        ...
        if (mCamera1 == null) {
            ...
            if (mHolder1 != null) {
                try {
                    setStartPreview(mCamera1, mHolder1); //讲相机与SurfaceView进行绑定
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
...
//    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        try {
            setStartPreview(mCamera1, mHolder1);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
